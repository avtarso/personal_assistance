...
from .models import Author, Tag, Quote, UploadFile
...
def top_10_tag_list(request):
    tags_with_counts = Tag.objects.annotate(num_quotes=Count('quote')).order_by('-num_quotes')[:10]
    context = {
...
def tag_quotes(request, tag_id):
    ...
    return render(request, 'quotes/tag_quotes.html', {'quotes': quotes, 'tag_name':tag.name})
...
def handle_uploaded_file_quotes(data, user, admin_user):
    for item in data:
        ...
        for tag_name in item['tags']:
            tag, created = Tag.objects.get_or_create(name=tag_name, defaults={'added_by': user if user.is_superuser else admin_user})
...
def upload_file_authors(request):
    return upload_file(request, handle_uploaded_file_authors, 'quotes/upload_authors.html')

def upload_file_quotes(request):
    return upload_file(request, handle_uploaded_file_quotes, 'quotes/upload_quotes.html')
